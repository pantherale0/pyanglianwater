...
@pytest.fixture
async def auth_instance():  # pylint: disable=redefined-outer-name
...
@pytest.fixture
def refresh_invalid_grant_response_text():  # pylint: disable=redefined-outer-name
...
@pytest.mark.asyncio
async def test_initial_auth_data(auth_instance):  # pylint: disable=redefined-outer-name
    ...
    with patch(  # pylint: disable=protected-access
        ...
        location = await auth_instance._get_initial_auth_data()  # pylint: disable=protected-access
        assert location == "https://example.com"


@pytest.mark.asyncio
async def test_submit_self_asserted_form(auth_instance):  # pylint: disable=redefined-outer-name
    ...
    with patch(  # pylint: disable=protected-access
        ...
        response = await auth_instance._submit_self_asserted_form(  # pylint: disable=protected-access
            "test_trans_id"
        )
...
@pytest.mark.asyncio
async def test_get_confirmation_redirect(auth_instance):  # pylint: disable=redefined-outer-name
...
async def test_get_token(auth_instance):  # pylint: disable=redefined-outer-name
    ...
    with patch(  # pylint: disable=protected-access
        ...
        token_data = await auth_instance._get_token("test_code")  # pylint: disable=protected-access
...
@pytest.mark.asyncio
async def test_send_login_request(auth_instance):  # pylint: disable=redefined-outer-name
    """Test the send_login_request method."""
    with (
        patch.object(
            auth_instance,
            "_get_initial_auth_data",  # pylint: disable=protected-access
            AsyncMock(return_value=("csrf_token", "trans_id")),
        ),
        patch.object(
            auth_instance, "_submit_self_asserted_form",  # pylint: disable=protected-access
            AsyncMock(return_value=True)
        ),
        patch.object(
            auth_instance,
            "_get_confirmation_redirect",  # pylint: disable=protected-access
            AsyncMock(
                return_value=(
                    "uk.co.anglianwater.myaccount://?code=test_code&state=test_state"
                )
            ),
        ),
        patch.object(
...
def test_raise_mapped_token_error_oauth_codes(error_code, expected_exception):
    ...
    with pytest.raises(expected_exception):
        auth._raise_mapped_token_error(  # pylint: disable=protected-access
            status=400, response_text=response_text
        )
...
def test_raise_mapped_token_error_real_invalid_grant_refresh_fixture(
    refresh_invalid_grant_response_text,  # pylint: disable=redefined-outer-name
):
